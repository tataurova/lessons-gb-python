...
def presence_response(presence_message):
    ...
    if ACTION in presence_message and \
        ...
        # заполняем лог
        res = 'args: ({}) = {}'.format(presence_message, {RESPONSE: 200})
        logger.info('{} - {} - {}'.format(res, presence_response.__name__, __name__))
        # Если всё хорошо шлем ОК
        return {RESPONSE: 200}
    else:
        # заполняем лог
        res = '({}) = {}'.format(presence_message, {RESPONSE: 400, ERROR: 'Не верный запрос'})
        logger.info('{} - {} - {}'.format(res, presence_response.__name__, __name__))
        # Шлем код ошибки
