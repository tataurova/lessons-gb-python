...
def create_presence(account_name="Guest"):
    ...
    # заполняем лог
    res = '= {}'.format(message)
    logger.info('{} - {} - {}'.format(res, create_presence.__name__, __name__))
...
def translate_message(response):
    ...
    # заполняем лог
    # 2018-06-06 20:37:44,622 - INFO - args: ({'response': 200},) = {'response': 200} - translate_response - client
    res = 'args: ({},) = {}'.format(response, response)
    logger.info('{} - {} - {}'.format(res, translate_message.__name__, __name__))
    # возвращаем ответ
    return response
